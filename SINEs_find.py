...
parser.add_argument("-g", "--ref_genome", choices=['mm39', 'mm10'],
                    help = "Set the reference genome you are working on.\
                        Default is mm39", default='mm39')
...
if args.ref_genome == "mm39":
    test_region = "chr11:101175523-101176023"  # Actb (beta-actin) gene - highly expressed
elif args.ref_genome == "mm10":
    test_region = "chr5:142903994-142904494"   # Gapdh - highly expressed
            ...
            if element[6] == '+':
                if sum(bw_plus.values(element[0], int(element[3]),int(element[4]))) \
                    > args.background * ((int(element[4]) - int(element[3])) \
                                         * peak_plus):
                    if "B2" in element[8] or "B3" in element[8]:
